        ...
        <div class="top1">
          ...
          <div class="inpDiv">
            <input
              ...
              placeholder="Enter contract addre"
            />
...
export default {
  ...
  created() {
    ...
    if (JSON.parse(localStorage.getItem("ETHlist"))) {
      this.ETHlist = JSON.parse(localStorage.getItem("ETHlist"));
    } else {
  ...
  methods: {
    ...
    getOtherERCSymbol(currentContract, address) {
      let that = this;
      let flag = true;
      currentContract.methods.symbol().call({ from: address }, function (error, result) {
        if (!error) {
          ...
          for (let index in that.ETHlist) {
            if (that.ETHlist[index].address == that.obj.address) {
              that.obj.status = true;
            }
          }
          that.searchBottom = that.obj;
    ...
    addSearch() {
      ...
      if (!that.searchBottom.status) {
        ...
        localStorage.setItem("selectETHlist", JSON.stringify(that.selectETHlist));
