...
export default {
  ...
  methods: {
    ...
    clickSure() {
      if (
        this.list1.every((v, i) => v == this.listWords[i]) &&
        this.list1.length != 0
      ) {
        //空数组使用every会一直得到true,但是空数组使用some却会得到false
        Toast.success(this.msg2);
        //需要把this.$store.state.walletType清空，不然进入钱包再备份，就会再次导入钱包
